...
$sql = "select time, date, nama_ruang FROM tempahan where id = $id";
 ...
 foreach($timew as $t){
	$t2 = explode("-",$t);
	$sql = "UPDATE jadual SET $t2[0]='DITEMPAH' WHERE date = '$row[1]' AND nama_ruang = '$row[2]'"; 
	
	mysql_query($sql);
 }
 mysql_query ("UPDATE tempahan SET status = 'TEMPAHAN BERJAYA' WHERE id='$id' ");
 mysql_query ("UPDATE laporan_tempahan SET bilangan_ditempah = bilangan_ditempah + 1 WHERE nama_ruang='$row[2]' ");
...
//mysql_query("UPDATE tempahan SET status = 'DITEMPAH' WHERE id='$id'");
...
<?php
            ...
            if($i%2==0){
				...
				echo "<b>STATUS TEMPAHAN:</b> <font color='green'>".$row2['status']."</font><br></div><br>";
				
				echo '<center><a href="lulus_tempahan_email.php?id='.$row2['id'].'" ><mark>HANTAR NOTIFIKASI</mark></a><b></center></div>';
